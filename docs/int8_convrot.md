...
- AMD RDNA2 (gfx1030-class) also uses the accelerated HIP path: hipblasGemmEx computes the I8 x I8 to I32 GEMM (verified on gfx1030 with ROCm 7.14 and newer) and is roughly 5.7x faster than the alternative at Z-Image shapes. A DP4A kernel is available as a fallback for ROCm stacks where the rocBLAS INT8 route does not work; build ggml with `GGML_CUDA_FORCE_I8_MMQ` to force it.
